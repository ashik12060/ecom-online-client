...
//         `${process.env.REACT_APP_API_URL}/api/category/New Arrival?page=${pageNumber}`
//       );
...
const NewArrival = () => {
  ...
  const fetchProducts = async (pageNumber = 1) => {
    ...
    try {
      ...
      setProducts(response.data.products || []);
      setTotalPages(response.data.totalPages || 1);
      // if you want to mark cache hits, you can include a `cached: true` in backend response
      setFromCache(response.data.cached || false);
    } catch (error) {
      toast.error("Failed to load New Arrival products");
    } finally {
      setLoading(false);
